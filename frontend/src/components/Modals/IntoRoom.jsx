...
function IntoRoom ({ onClose, room }){
  ...
  return (
    <div
      className={`${style.openModal} ${style.modal}`}
      onClick={onClose}
    >
      <div className={style.modalForm} onClick={handleClose}>
            ...
            <div style={{display : "inline-block"}} onClick={handleMic}>
              {isMic ? (<button className={style.vbtn}> <Mic className={style.buttonicon} width="20" height="20"></Mic> 음소거 </button>) 
                                : (<button className={style.vbtn}> <Micx className={style.buttonicon} width="20" height="20" 
                                filter="invert(34%) sepia(54%) saturate(4911%) hue-rotate(327deg) brightness(100%) contrast(103%)"></Micx> 음소거 해제 </button>)}

            </div>
            <div style={{display : "inline-block"}} onClick={handleVideo}>
              {isVideo ? (<button className={style.vbtn}> <Video className={style.buttonicon} width="20" height="20"></Video> 비디오 중지 </button>) 
                                : (<button className={style.vbtn}> <Videox className={style.buttonicon} width="20" height="20" 
                                filter="invert(34%) sepia(54%) saturate(4911%) hue-rotate(327deg) brightness(100%) contrast(103%)"></Videox> 비디오 시작 </button>)}
